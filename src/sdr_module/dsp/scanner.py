...
class FrequencyScanner:
    ...
    def _advance_step(self) -> None:
        ...
        if self._config.direction == ScanDirection.UP:
            self._current_freq = (
                self._config.start_freq_hz + self._step_index * self._config.step_hz
            )
        else:
            self._current_freq = (
                self._config.end_freq_hz - self._step_index * self._config.step_hz
            )
        ...
        self._current_freq = max(
            ...
            min(
                self._current_freq,
                max(self._config.start_freq_hz, self._config.end_freq_hz),
            ),
        )
    ...
    def _make_status(self) -> ScanStatus:
        ...
        progress = (
            (self._step_index / self._total_steps) * 100 if self._total_steps > 0 else 0
        )
    ...
    def get_result(self) -> ScanResult:
        ...
        return ScanResult(
            ...
            scan_time_sec=(
                time.time() - self._scan_start_time if self._scan_start_time else 0.0
            ),
        )
